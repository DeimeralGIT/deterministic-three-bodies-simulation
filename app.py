...
<div id="app">
  <aside class="panel">
    ...
    <label for="probabilisticBehaviour">Probabilistic behaviour <span id="probabilisticBehaviourValue">0.0000</span></label>
    <input id="probabilisticBehaviour" type="range" min="0" max="0.01" step="0.0001" value="0">
    ...
    <div id="viewport">
      <div id="indicator">
        <div class="indicator-head">
          <span>Bodies</span>
          <button id="indicatorToggle" type="button" aria-label="Collapse indicator">
            <span class="indicator-arrow" aria-hidden="true"></span>
          </button>
        </div>
...
const state = {
  ...
  indicatorRenderCollapsed: false,
};
...
const ui = Object.fromEntries(
  ["bodyCount", "gravity", "limitX", "limitY", "limitZ", "showGrid", "showAxisNumbers", "stepsPerFrame", "trailLength", "probabilisticBehaviour"].map((id) => [id, document.getElementById(id)])
);
...
function readConfig() {
  ...
  const probabilisticBehaviour = Math.round(clamp(Number(ui.probabilisticBehaviour.value), 0, 0.01) * 10000) / 10000;
  ui.limitX.value = String(limitX);
  ui.limitY.value = String(limitY);
  ui.limitZ.value = String(limitZ);
  ui.probabilisticBehaviour.value = probabilisticBehaviour.toFixed(4);
...
function updateLabels() {
  ...
  document.getElementById("probabilisticBehaviourValue").textContent = Number(ui.probabilisticBehaviour.value).toFixed(4);
...
function setIndicatorUiState() {
  const indicator = document.getElementById("indicator");
  const toggle = document.getElementById("indicatorToggle");
  indicator.classList.toggle("collapsed", state.indicatorCollapsed);
  toggle.classList.toggle("collapsed", state.indicatorCollapsed);
  toggle.setAttribute("aria-label", state.indicatorCollapsed ? "Expand indicator" : "Collapse indicator");
}

function renderIndicatorRows() {
  const rows = document.getElementById("indicatorRows");
  rows.innerHTML = "";
  for (let i = 0; i < state.bodies.length; i++) {
    ...
    if (state.indicatorRenderCollapsed) {
      row.innerHTML = `
...
function toggleIndicatorCollapsedAnimated() {
  if (state.indicatorAnimating) return;
  const indicator = document.getElementById("indicator");

  const willCollapse = !state.indicatorCollapsed;
  state.indicatorCollapsed = willCollapse;

  if (!willCollapse) {
    // Expand: show full content before panel grows.
    state.indicatorRenderCollapsed = false;
  }
  ...
  const finish = () => {
    if (!state.indicatorAnimating) return;
    state.indicatorAnimating = false;
    if (willCollapse) {
      // Collapse: switch to compact rows only after panel shrink finishes.
      state.indicatorRenderCollapsed = true;
      setIndicatorUiState();
      renderIndicatorRows();
    }
  ...
  setTimeout(finish, INDICATOR_ANIM_MS + 120);
}
...
function applyProbabilisticBehaviour(bodies, config) {
  const intensity = Math.max(0, Math.min(0.01, config.probabilisticBehaviour));
  if (intensity <= 1e-9) return;

  // Blend direction and magnitude jitter to keep behavior visible but controllable.
  const directionMaxAngle = 0.35 * intensity;
  const speedJitter = 0.12 * intensity;

  for (const body of bodies) {
    const speed = body.velocity.length();
    if (speed < 1e-9) continue;

    const jitterDirection = Math.random() < 0.5;
    if (jitterDirection) {
      const randomAxis = randomDirection(Math.random);
      const angle = (Math.random() * 2 - 1) * directionMaxAngle;
      body.velocity.applyAxisAngle(randomAxis, angle);
    } else {
      const scale = 1 + (Math.random() * 2 - 1) * speedJitter;
      body.velocity.multiplyScalar(Math.max(0.05, scale));
    }
  }
}
...
function tick() {
  const config = readConfig();
  const probabilistic = Math.max(0, Math.min(0.01, config.probabilisticBehaviour));
  if (state.running && !state.escaped) {
    for (let i = 0; i < config.stepsPerFrame; i++) {
      rk4Step(state.bodies, config);
      if (probabilistic > 1e-9) applyProbabilisticBehaviour(state.bodies, config);
      // Keep the system barycenter fixed at sandbox center to avoid global drift wall hits.
      applyComFrame(state.bodies);
      state.step += 1;
...
document.getElementById("indicatorToggle").addEventListener("click", () => {
  toggleIndicatorCollapsedAnimated();
});
...
<style>
  * { box-sizing: border-box; }
  ...
  #indicator { position: absolute; left: 12px; bottom: 12px; width: min(380px, calc(100% - 24px)); padding: 10px 12px; border: 1px solid rgba(23,32,31,0.16); border-radius: 8px; background: rgba(255,255,255,0.86); backdrop-filter: blur(8px); color: #17201f; font-family: ui-monospace, SFMono-Regular, Menlo, Consolas, monospace; font-size: 12px; line-height: 1.35; pointer-events: auto; overflow-x: auto; overflow-y: hidden; transition: width 1s ease, padding 1s ease; }
  #indicator.collapsed { width: min(140px, calc(100% - 24px)); }
  .indicator-head { display: flex; align-items: center; justify-content: space-between; gap: 10px; margin-bottom: 8px; font-family: Inter, ui-sans-serif, system-ui, sans-serif; font-weight: 700; }
  #indicatorRows { transition: opacity 1s ease; }
  #indicator.collapsed #indicatorRows { opacity: 0.98; }
  #indicatorToggle { height: 24px; width: 24px; border-radius: 999px; border: 0; background: #2a6f66; color: #fff; font-size: 11px; font-weight: 700; cursor: pointer; padding: 0; display: inline-flex; align-items: center; justify-content: center; }
  #indicatorToggle:hover { background: #245e57; }
  .indicator-arrow { width: 0; height: 0; border-top: 5px solid transparent; border-bottom: 5px solid transparent; border-left: 8px solid #ffffff; transform: rotate(180deg); transition: transform 1s ease; transform-origin: 45% 50%; }
  #indicatorToggle.collapsed .indicator-arrow { transform: rotate(0deg); }
  .indicator-row { display: grid; grid-template-columns: 14px 58px repeat(4, auto); gap: 9px; align-items: center; white-space: nowrap; min-width: max-content; }
